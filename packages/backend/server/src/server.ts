...
export async function run() {
  ...
  app.useBodyParser('raw', { limit: 1024 * OneMB }); // 1GB - Increased for personal use
  ...
  app.use(
    graphqlUploadExpress({
      maxFileSize: 1024 * OneMB, // 1GB - Increased for personal use
      maxFiles: 100, // Increased from 32
    })
  );
